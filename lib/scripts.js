...
var scripts = {
  ...
  takeLock: function(queue, job, renew, ensureActive){
    if (renew) console.log('attempting renew lock for job id #', job.jobId)
    else console.log('attempting lock for job id #', job.jobId)
    ...
    if (ensureActive) {
      var isJobInList = this._isJobInList.replace(/KEYS\[1\]/, '"' + job.queue.toKey('active') + '"')
        .replace('return item', 'if item')
        .replace(/ARGV\[1\]\)/, '"' + job.jobId + '")');
    ...
    redlock.on('clientError', function(err) { console.log('clientError', err) });
    return redlock.lock(job.lockKey(), queue.LOCK_RENEW_TIME);
  },

  releaseLock: function(job){
    console.log('attempting to release lock for job id #', job.jobId)
